...
namespace SENAME.Senainfo.ModFichaResidencial.DAL.DAO
{
    ...
    {
        public DataTable ObtenerParSalud()
        {
            ...
            try
            {
                ...
                {
                    con.Open();
                    using (var cmd = new SqlCommand("FichaRes.Get_parSalud", con))
                    {
            ...
            catch (Exception e)
            {
                ...
                if (glosaError == "" || glosaError == null) glosaError = "Se ha producido una excepción de sistema no recuperable desde el servidor datos. Informar al adminitrador (se recomienda enviar una impresión de pantalla del error desplegado). Método: ObtenerParSalud";
