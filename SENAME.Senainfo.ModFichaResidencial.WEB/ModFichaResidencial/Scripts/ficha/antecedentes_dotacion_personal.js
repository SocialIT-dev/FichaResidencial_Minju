...
// Ini Polo
function CargaParValores1HAR(CodFicha, CodEstadoFichaAUX) {
    ...
    $.ajax({
        type: "POST",
        url: "FichaResidencial.aspx/ObtenerParValoresHAR",
        data: parametros,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (r) { 
            $('#gridDotacion').DataTable({
                destroy: true,
                paging: false,
                searching: false,
                lengthChange: false,
                ...
                data: r.d,
                columns: [
                    { "data": "D_Descripcion", "sClass": "etiqCampo3" },
                    { "data": "Select_Profesion"},
                    { "data": "Input_Cantidad" },
                    { "data": "Select_CodJornada" },
                    { "data": "Input_HorasSemanales" }
                ]
            ...
            if (CodEstadoFichaAUX == 0) {
                ...
                $("#dotacion_101_Observaciones").val(r.d[0].D_Observaciones);
            }
...
function GrabarAntecedentesDotacionPersonalHAR() {
    ...
    strGrilla = strGrilla.replace(/~"}/g, "\"}");
        ...
        $.each(r.d,
            function () {
                ...
                //ActivarDesactivarBotonesGrabar(3, true);
            }
        );
...
//                //Agregado
//                $("#ValorCodProfesion_1").val(this.Cantidad);
...
//FUNCIONES FRONTEND
//Hector
function HabilitaDotacionPersonalV2(id) {
    var idAux = id.split("_");
    var id_ = idAux[1];
  
    var IdCantidad_ = $("#IdCantidad_" + id_).val();
    var IdCodJornada_ = $("#IdCodJornada_" + id_).val();
    var IdHorasSemanales_ = $("#IdHorasSemanales_" + id_).val();

    if ((IdCantidad_ == "0" || IdCantidad_ == "") && (IdCodJornada_ == "0" || IdCodJornada_ == "") && (IdHorasSemanales_ == "0" || IdHorasSemanales_ == "")) {
        document.getElementById("IdProfesion_" + id_).selectedIndex = 0;
    }
    else {
        document.getElementById("IdProfesion_" + id_).selectedIndex = 1;
    }
}
