...
namespace SENAME.Senainfo.ModFichaResidencial.WEB.ModFichaResidencial
{
    public partial class FichaResidencial : System.Web.UI.Page
    {
        ...
        //public static List<ResultadoOperacionPersonalDtoHAR> GrabarAntecedentesDotacionPersonalHAR(
        //                                                                                    int? CodFicha,
        //                                                                                    int? CodProyecto,
        //                                                                                    int? CodEstadoFicha,
        //                                                                                    int? idUsuarioActualizacion,
        ...
        [WebMethod]
        public static List<ResultadoOperacionPersonalDtoHAR> GrabarAntecedentesDotacionPersonalHAR(string strGrilla)
        {
            var CodFicha = 0;
            var CodProyecto = 0;
            var CodEstadoFicha = 0;
            ...
            string[] grilla1 = strGrilla.Split('~');


            var result = new List<ResultadoOperacionPersonalDtoHAR>();
            foreach (var grill in grilla1)
            {
                string[] grilla2 = grill.Split('|');

               
                CodFicha = Convert.ToInt32(grilla2[0]);
           

     
                if (CodFicha == 0)
                    {
                        CodFicha = CodFichaAux2;
                    }

                    CodProyecto = Convert.ToInt32(grilla2[1]);
                    CodEstadoFicha = Convert.ToInt32(grilla2[2]);
                    idUsuarioActualizacion = Convert.ToInt32(grilla2[3]);
                    CodProfesion = Convert.ToInt32(grilla2[4]);
                    Cantidad = Convert.ToInt32(grilla2[5]);
                    CodJornada = Convert.ToInt32(grilla2[6]);
                    HorasSemanales = Convert.ToInt32(grilla2[7]);
                    Observaciones = grilla2[8];

                    ResultadoOperacionPersonalImplHAR _resultadoOperacionPersonalImplHAR = new ResultadoOperacionPersonalImplHAR();
                    result = _resultadoOperacionPersonalImplHAR.GrabarAntecedentesPersonalHAR(CodFicha, CodProyecto, CodEstadoFicha, idUsuarioActualizacion, CodProfesion, Cantidad, CodJornada, HorasSemanales, Observaciones);



                    foreach (ResultadoOperacionPersonalDtoHAR dto in result)
                    {
                        //if (this.REGISTRO_ACTUALIZADO != -1 && this.REGISTRO_ACTUALIZADO != null) {
                        if (dto.REGISTRO_ACTUALIZADO != "-1" && dto.REGISTRO_ACTUALIZADO != null)
                        {
                            CodFichaAux2 = Convert.ToInt32(dto.REGISTRO_ACTUALIZADO);
                        }
                        else
                        {
                            if (dto.ERROR_MESSAGE_ != null)
                            {
                                msjerror1 = msjerror1 + dto.ERROR_MESSAGE_ + "\n";
                            }
                            else
                            {
                                msjerror2 = msjerror2 + dto.error + "\n";
                            }
                    ...
                    foreach (ResultadoOperacionPersonalDtoHAR dto2 in result)
                    {
                        dto2.ERROR_MESSAGE_ = msjerror1;
                        dto2.error = msjerror2;
        ...
        [WebMethod]
        public static List<GetParValoresHARDto> ObtenerParValoresHAR(int CodFicha)
        {
            GetParDotacionPersonalHARImpl _dotacionPersonalImplHAR = new GetParDotacionPersonalHARImpl();
            var result = _dotacionPersonalImplHAR.ObtenerParDotacion(CodFicha);
            var strSelectProfesion = "";
            var strSelectJornada = "";
            foreach (GetParValoresHARDto dto in result){

                if (dto.D_Cantidad !=0)
                {
                    strSelectProfesion = "<option value = '0'> NO </option> <option value = '1' selected> SI </option>";
                }
                else
                {
                    strSelectProfesion = "<option value = '0' selected> NO </option> <option value = '1'> SI </option>";
                }
               
                dto.Select_Profesion = "<select id=IdProfesion_"+ dto.D_CodProfesion + " disabled='disabled'  class='form-control textCampoSel1 dllSiNo' >" + strSelectProfesion + "</select>";


                dto.Input_Cantidad = "<input id = IdCantidad_"+ dto.D_CodProfesion + " class='form-control textCampo3' value="+dto.D_Cantidad.ToString()+ " maxlength='4' placeholder='' onkeypress='return ValidaIngresoSoloNumeros(this.value, event);' onkeyup='HabilitaDotacionPersonalV2(this.id);' />";


 

                switch (dto.D_CodJornada)
                {
                    case 0:
                        strSelectJornada = "<option value = '0' selected>Selecionar</option> <option value = '1'>COMPLETA</option> <option value = '2'>PARCIAL</option> <option value = '3' >OTRA</option> ";
                        break;
                    case 1:
                        strSelectJornada = "<option value = '0' >Selecionar</option> <option value = '1' selected>COMPLETA</option> <option value = '2'>PARCIAL</option> <option value = '3' >OTRA</option> ";
                        break;
                    case 2:
                        strSelectJornada = "<option value = '0' >Selecionar</option> <option value = '1'>COMPLETA</option> <option value = '2' selected>PARCIAL</option> <option value = '3' >OTRA</option> ";
                        break;


                    default:
                        strSelectJornada = "<option value = '0' >Selecionar</option> <option value = '1'>COMPLETA</option> <option value = '2'>PARCIAL</option> <option value = '3' selected >OTRA</option> ";
                        break;
                }

                dto.Select_CodJornada= "<select id=IdCodJornada_" + dto.D_CodProfesion + "  class = 'form - control textCampoSel1'  onchange='HabilitaDotacionPersonalV2(this.id);'>" + strSelectJornada + "</select>";

                dto.Input_HorasSemanales = "<input id = IdHorasSemanales_" + dto.D_CodProfesion + " class='form-control textCampo3' value="+dto.D_HorasSemanales.ToString()+ " maxlength='2' placeholder='' onkeypress='return ValidaIngresoSoloNumeros(this.value, event);' onkeyup='HabilitaDotacionPersonalV2(this.id);'/>";
